/* unicast-server.c - a datagram socket 'server'
...
struct packet /*Structure to hold send/receive sntp buffer*/
{
	uint8_t flags;
	uint8_t stratum;
	uint8_t poll;
...
int main( void) 
{
	...
	while(1)
	{
		LOOP: addr_len = sizeof( struct sockaddr);
		...
		switch (numbytes)
		{
			case 48:break; //packet without optional fields
			case 52:break; //packet with key identifier
			case 64:break; //packet with message digest
			case 68:break; //packet with both key identifier and key digest
			default: goto LOOP; break; //drop packet
		}
		
		if (request_packet.flags & 0x03) //checks if mode is set to Client
		{
			gettimeofday(&receive_tv, NULL);
			printf( "Got packet from %s\n", inet_ntoa( their_addr.sin_addr));
			printf( "Packet is %d bytes long\n", numbytes);
	
			response_packet.flags = 0x24; //mode is client, ipv4
			response_packet.stratum = 1;
			response_packet.poll = request_packet.poll;
			gettimeofday(&reference_tv, NULL);
			response_packet.reference_timestamp = htonl(reference_tv.tv_sec + 2208988800); //adds on seconds (1900 - 1970)
